###
#
# Lenovo Redfish examples - Remove ssh pubkey
#
# Copyright Notice:
#
...
function lenovo_remove_ssh_pubkey
{
    <#
   .Synopsis
    Cmdlet used to remove ssh pubkey
   .DESCRIPTION
    Cmdlet used to lremove ssh pubkey from BMC using Redfish API. Set result will be printed to the screen. Connection information can be specified via command parameter or configuration file.
   ...
   .EXAMPLE
    lenovo_remove_ssh_pubkey -ip 10.10.10.10 -username USERID -password PASSW0RD -destusername NEWUSERNAME
   #>
    ...
    try
    {
        ...
        if($converted_object.'@odata.etag' -ne $null)
        {
            ...
            $JsonBody = '{"Oem":{"Lenovo":{"SSHPublicKey":[]}}}'
        }
        else
        {
            $JsonHeader = @{ "If-Match" = ""
        ...
        Write-Host
                [String]::Format("- PASS, statuscode {0} returned successfully to delete pubsshkey",$response.StatusCode)
        return $True
    }
    catch
    {
