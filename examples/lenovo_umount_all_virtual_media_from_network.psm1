###
#
# Lenovo Redfish examples - UMount virtual media from network
# Copyright Notice:
#
...
function lenovo_umount_all_virtual_media_from_network
{
    <#
   .Synopsis
    Cmdlet used to umount virtual media from network
   .DESCRIPTION
    Cmdlet used to umount virtual media from network from BMC using Redfish API. Set result will be printed to the screen. Connection information can be specified via command parameter or configuration file.
    - ip: Pass in BMC IP address
    - username: Pass in BMC username
    - password: Pass in BMC username password
    - config_file: Pass in configuration file path, default configuration file is config.ini
   .EXAMPLE
    lenovo_umount_all_virtual_media_from_network -ip 10.10.10.10 -username USERID -password PASSW0RD
   #>
    ...
    try
    {
        ...
        foreach ($manager_url_string in $manager_url_collection)
        {
            ...
            $umount_image_url = "https://$ip" + $converted_object.Actions.'#LenovoRemoteMapService.UMount'.'target'
            
            # uMount virtual media from network
            $response = Invoke-WebRequest -Uri $umount_image_url -Headers $JsonHeader -Method Post -UseBasicParsing
            
            Write-Host
            [String]::Format("- PASS, statuscode {0}. All Media File from Network umount successfully.", $response.StatusCode)
