###
#
# Lenovo Redfish examples - Update BMC User Role
#
# Copyright Notice:
#
...
function lenovo_update_user_role
{
   <#
   .Synopsis
    Cmdlet used to update BMC user role
   .DESCRIPTION
    Cmdlet used to update BMC user role using Redfish API
    ...
    - role_id: Pass in BMC user role id
    - config_file: Pass in configuration file path, default configuration file is config.ini
   .EXAMPLE
    lenovo_get_bmc_user_accounts -ip 10.10.10.10 -username USERID -password PASSW0RD -userid XXX -role_id XXX
   #>
    ...
    try
    {
        ...
        foreach ($i in $hash_table.Members)
        {
            ...
            if($hash_table.UserName -eq $userid)
            {
                ...
                Write-Host
                [String]::Format("- PASS, statuscode {0} returned successfully to changed {1} role",$response.StatusCode,$hash_table.UserName)
