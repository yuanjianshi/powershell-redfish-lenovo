...
function lenovo_mount_virtual_media_from_network
{
    <#
   .Synopsis
    Cmdlet used to mount virtual media from network
   .DESCRIPTION
    Cmdlet used to mount virtual media from network from BMC using Redfish API. Set result will be printed to the screen. Connection information can be specified via command parameter or configuration file.
   ...
   .EXAMPLE
    lenovo_mount_virtual_media_from_network -ip 10.10.10.10 -username USERID -password PASSW0RD -fsprotocol FSPROTOCOL -fsip FSIP -fsusername USERNAME -fspassword FSPASSWORD -fsdir FSDIR -isoname ISONAME
   #>
    ...
    try
    {
        ...
        foreach ($manager_url_string in $manager_url_collection)
        {
            ...
            $images_member_url = "https://$ip" + $converted_object.MountImages.'@odata.id'
            ...
            {
                write-Host 'Mount media iso network only support protocol Samba, NFS, HTTP/HTTPS, SFTP/FTP'
                return $False
            }
            ...
            # Add image member
            $response = Invoke-WebRequest -Uri $images_member_url -Headers $JsonHeader -Method Post -Body $JsonBody -ContentType 'application/json' -UseBasicParsing
            if($response.statuscode -eq 201)
            {
                # Mount virtual media from network
