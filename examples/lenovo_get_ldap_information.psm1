###
#
# Lenovo Redfish examples - Get ldap information
# Copyright Notice:
#
...
function lenovo_get_ldap_information
{
   <#
   .Synopsis
    Cmdlet used to get ldap information
   .DESCRIPTION
    Cmdlet used to get ldap information using Redfish API
   ...
   .EXAMPLE
    lenovo_get_ldap_information -ip 10.10.10.10 -username USERID -password PASSW0RD
   #>
    ...
    try
    {
        ...
        foreach ($manager_url_string in $manager_url_collection)
        {
            ...
            $response = Invoke-WebRequest -Uri $ldap_client_uri -Headers $JsonHeader -Method Get -UseBasicParsing
            $converted_object = $response.Content | ConvertFrom-Json
            $hash_table = @{}
            $converted_object.psobject.properties | Foreach { $hash_table[$_.Name] = $_.Value }

            # Get the ldap client information
            $ldap_client_info = @{}
            foreach($key in $hash_table.keys)
            {
                if('Description','@odata.etag','@odata.id','@odata.type','@odata.context' -notcontains $key)
                {
                    $ldap_client_info[$key] = $hash_table.$key
                }
            }
            $ldap_client_info
