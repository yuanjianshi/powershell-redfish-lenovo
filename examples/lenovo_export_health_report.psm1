...
function lenovo_export_health_report
{
   <#
   .Synopsis
    Cmdlet used to export health report
   .DESCRIPTION
    Cmdlet used to export health report using Redfish API
   ...
   .EXAMPLE
    lenovo_export_health_report -ip 10.10.10.10 -username USERID -password PASSW0RD
   #>
    ...
    try
    {
        ...
        foreach ($manager_url_string in $manager_url_collection)
        {
            ...
            $converted_object = $response.Content | ConvertFrom-Json
            $uri_health_report = "https://$ip" + $converted_object.'Actions'.'#LenovoServiceData.ExportHealthReport'.'target'
            
            # Send requests to get health report file
            try
            {
                $releaseDate = Get-Date -Format 'yyyyMdHm'
                $health_report_name = $ip+'_'+"health_report_"+ $releaseDate+'.xml'
                $response = Invoke-WebRequest -Uri $uri_health_report -Headers $JsonHeader -Method Post -UseBasicParsing -ContentType 'application/json'
                $converted_object = $response.Content | ConvertFrom-Json
                $converted_object.Report | Out-File $health_report_name
            }
            catch
            {
            ...
            [String]::Format("-PASS, statuscode {0}, The health report is saved as {1}", $response.StatusCode, $health_report_name)
            return $True
        }
    }    
    catch
