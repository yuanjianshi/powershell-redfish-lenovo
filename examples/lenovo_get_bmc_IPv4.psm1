###
#
# Lenovo Redfish examples - Get BMC IPv4
# Copyright Notice:
#
...
function lenovo_get_bmc_ipv4
{
    <#
   .Synopsis
    Cmdlet used to get BMC IPv4
   .DESCRIPTION
    Cmdlet used to get BMC IPv4 from BMC using Redfish API. Set result will be printed to the screen. Connection information can be specified via command parameter or configuration file.
    - ip: Pass in BMC IP address
    - username: Pass in BMC username
    - password: Pass in BMC username password
    - config_file: Pass in configuration file path, default configuration file is config.ini
   .EXAMPLE
    lenovo_get_bmc_ipv4 -ip 10.10.10.10 -username USERID -password PASSW0RD
   #>
        ...
        foreach ($manager_url_string in $manager_url_collection)
        {
            ...
            foreach ($i in $member_list)
            {
                ...
                if($member_array -contains "NIC")
                {
                    ...
                    $converted_object = $response.Content | ConvertFrom-Json
                    $ipv4_info = @{}
                    $ipv4_info."IPv4Addresses" = $converted_object."IPv4Addresses"
                    $ipv4_info."IPv4AddressAssignedby" = $converted_object."Oem"."Lenovo"."IPv4AddressAssignedby"
                    $ipv4_info
                    Write-Host " "
                }
